...
function PlaceDetails() {
	...
	const [place, setPlace]: any = useState(null)

	useEffect(() => {
		const fetchData = async () => {
			const response: any = await fetch(`http://localhost:5000/places/${placeId}`)
	...
	async function deleteComment(deletedComment: { commentId: any; }) {
	...
	if (place.comments.length) {
		...
		comments = place.comments.map((comment: { commentId: React.Key | null | undefined; }) => {
			return (
